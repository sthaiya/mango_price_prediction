...
packages =c("tidyverse", "caret", "skimr", "corrplot", "googledrive", "janitor", "naniar", 
            "data.validator", "CatEncoders", "randomForest", "xgboost", "import")
...
# We will keep variety
# filter(variety != "Other") %>% # remove where variety is other. We keep variety
  
# Step 2: Identify duplicates -- Keep the dupes due to frequencies
...
# mango <- mango %>%
#   mutate(
#     state_orig = state, district_orig = district, market_orig = market, variety_orig = variety,
#     state = transform(LabelEncoder.fit(state), state),
#     district = transform(LabelEncoder.fit(district), district),
#     market = transform(LabelEncoder.fit(market), market),
#     variety = transform(LabelEncoder.fit(variety), variety),
...
#================== CREATE META DATA FOR UI ====================================
states <- mango[c('state', 'state_orig')] %>% distinct(.keep_all = TRUE) %>%
  rename(key = state_orig, value = state) %>% mutate(type = "state")

districts <- mango[c('district', 'district_orig')] %>% distinct(.keep_all = TRUE) %>%
  rename(key = district_orig, value = district) %>% mutate(type = "district")

markets <- mango[c('market', 'market_orig')] %>% distinct(.keep_all = TRUE) %>%
  rename(key = market_orig, value = market) %>% mutate(type = "market")

varieties <- mango[c('variety', 'variety_orig')] %>% distinct(.keep_all = TRUE) %>%
  rename(key = variety_orig, value = variety) %>% mutate(type = "variety")
meta_data <- rbind(states,districts, markets, varieties)

meta_file_path = paste0(script_path(), .Platform$file.sep, "meta_data.csv")
write.csv(meta_data, file = meta_file_path, row.names = FALSE)
#====================== END OF UI META DATA ====================================
...
cols_to_drop = c("arrival_date", "update_date", "min_price", "max_price", "state_orig",
                 "district_orig", "market_orig", "variety_orig")
...
# Random Forest will use caret for this training
...
rf_model <- train(modal_price ~ ., data = train,  method = "rf",  trControl = tr_ctrl)
print(rf_model)
...
# XGBoost
# test_input_x <- as.matrix(select(test, -modal_price))
# test_input_y <- test$modal_price

#"arrival_date" not yet removed
# Specifying evaluation metrics and display training and testing data at each of the 100 rounds (using RMSE)
# d_train_data = xgb.DMatrix(data=input_x, label = input_y)
# d_test_data = xgb.DMatrix(data=test_input_x, label = test_input_y)
#   
...
# final_xgb = xgboost(data=d_train_data, max.depth=6, nrounds = 450, verbose = FALSE)
# rmse <- RMSE(test_input_y, )
...
saveRDS(xgb_model, paste0(script_path(), .Platform$file.sep, "xgb_model.rds"))

saveRDS(rf_model, paste0(script_path(), .Platform$file.sep, "rf_model.rds"))
